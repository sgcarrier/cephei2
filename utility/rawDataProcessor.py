...
class RawDataProcessor():
    ...
    PROCESSED_TIMESTAMP_FRAME_FORMAT_WRAW = {'RAW': {'dtype': 'u8', 'offset': 0, 'bitMask':    0xFFFFFFFFFFFFFFFF, 'bitLen':  64},
                                  'RESERVED':    {'dtype': 'u4', 'offset': 55, 'bitMask':    0x1FF, 'bitLen': 9},
                                  'Timestamp':  {'dtype': 'u8', 'offset': 17, 'bitMask': 0xFFFFFFFFFF, 'bitLen': 38},
                                  'Energy':  {'dtype': 'u4', 'offset':  9, 'bitMask':     0xFF, 'bitLen':  8},
                                  'Addr':    {'dtype': 'u4', 'offset':  0, 'bitMask':    0x1FF, 'bitLen':  9}}
    ...
    def getFrameFormat(self, num, keepRaw=False):
        if num == 0:
            self.format_reverse_bits = False
            if keepRaw:
                return self.RAW_TIMESTAMP_FRAME_FORMAT_WRAW
            else:
                return self.RAW_TIMESTAMP_FRAME_FORMAT
        elif num == 1:
            self.format_reverse_bits = True
            if keepRaw:
                return self.PLL_TDC_FRAME_FORMAT_WRAW
            else:
                return self.PLL_TDC_FRAME_FORMAT
        elif num == 2:
            self.format_reverse_bits = False
            if keepRaw:
                return self.PROCESSED_TIMESTAMP_FRAME_FORMAT_WRAW
            else:
                return self.PROCESSED_TIMESTAMP_FRAME_FORMAT
        elif num == 3:
            self.format_reverse_bits = False
            if keepRaw:
                return self.QKD_FRAME_FORMAT_WRAW
            else:
    ...
    def getFrameDtype(self, num, keepRaw=False):
        if num == 0:
            if keepRaw:
                return self.RAW_TIMESTAMP_FRAME_DTYPE_WRAW
            else:
                return self.RAW_TIMESTAMP_FRAME_DTYPE
        elif num == 1:
            if keepRaw:
                return self.PLL_TDC_FRAME_DTYPE_WRAW
            else:
                return self.PLL_TDC_FRAME_DTYPE
        elif num == 2:
            if keepRaw:
                return self.PROCESSED_TIMESTAMP_FRAME_DTYPE_WRAW
            else:
                return self.PROCESSED_TIMESTAMP_FRAME_DTYPE
        elif num == 3:
            if keepRaw:
                return self.QKD_FRAME_DTYPE_WRAW
            else:
                return self.QKD_FRAME_DTYPE
        else:
            return -1
    ...
    def raw2compArray(self, data, frameFormatNum, keepRaw=False):
        format = self.getFrameFormat(frameFormatNum, keepRaw=keepRaw)
        if (format == -1):
        ...
        dtype= self.getFrameDtype(frameFormatNum, keepRaw=keepRaw)
